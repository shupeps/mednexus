...
function HomePage() {
  ...
  const handleProtectedNavigation = (path, requiredUserType) => {
    if (!currentUser) {
      alert('Você precisa estar logado para acessar esta página.');
      navigate('/login');
    } else if (requiredUserType && currentUser.userType !== requiredUserType) {
      alert(`Apenas ${requiredUserType}s podem acessar esta página.`);
      navigate('/');
    } else {
      navigate(path);
    }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex items-center space-x-4">
              {currentUser ? (
                <Button variant="ghost" onClick={handleLogout}>Sair ({currentUser.email})</Button>
              ) : (
                <>
            ...
            <div className="md:hidden py-4 border-t">
              <nav className="flex flex-col space-y-4">
                ...
                <div className="flex flex-col space-y-2 pt-4">
                  {currentUser ? (
                    <Button variant="ghost" className="justify-start" onClick={handleLogout}>Sair ({currentUser.email})</Button>
                  ) : (
                    <>
                      <Link to="/login"><Button variant="ghost" className="justify-start">Entrar</Button></Link>
                      <Link to="/cadastro"><Button className="justify-start">Cadastrar</Button></Link>
                    </>
                  )}
                </div>
              </nav>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <div className="mt-8 flex flex-col sm:flex-row gap-4">
                <Button size="lg" className="bg-blue-600 hover:bg-blue-700" onClick={() => handleProtectedNavigation('/vagas', 'medico')}>
                  Buscar Vagas
                  <ChevronRight className="w-5 h-5 ml-2" />
                </Button>
                <Button size="lg" variant="outline" onClick={() => handleProtectedNavigation('/publicar-vaga', 'instituicao')}>
                  Sou Estabelecimento
                </Button>
              </div>
            </div>
            ...
            {displayedVagas.map((vaga) => (
              <Card key={vaga.id} className="hover:shadow-lg transition-shadow cursor-pointer">
                ...
                <CardContent>
                  ...
                  <Button className="w-full mt-4" variant="outline" onClick={() => handleProtectedNavigation('/vagas', 'medico')}>
                    Ver Detalhes
                  </Button>
                </CardContent>
              </Card>
      ...
      <section className="py-16 bg-blue-600">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button size="lg" variant="secondary" onClick={() => handleProtectedNavigation('/cadastro', 'medico')}>
              Cadastrar como Médico
            </Button>
            <Button size="lg" variant="outline" className="text-white border-white hover:bg-white hover:text-blue-600" onClick={() => handleProtectedNavigation('/publicar-vaga', 'instituicao')}>
              Sou Estabelecimento
            </Button>
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-gray-400">
                <li><a href="#" onClick={() => handleProtectedNavigation('/vagas', 'medico')} className="hover:text-white transition-colors">Buscar Vagas</a></li>
                <li><a href="#" onClick={() => handleProtectedNavigation('/cadastro', 'medico')} className="hover:text-white transition-colors">Criar Perfil</a></li>
            ...
            <div>
              ...
              <ul className="space-y-2 text-gray-400">
                <li><a href="#" onClick={() => handleProtectedNavigation('/publicar-vaga', 'instituicao')} className="hover:text-white transition-colors">Publicar Vaga</a></li>
                <li><a href="#" onClick={() => alert('Funcionalidade de Buscar Médicos em desenvolvimento!')}>Buscar Médicos</a></li>
                <li><a href="#" className="hover:text-white transition-colors" onClick={() => alert('Funcionalidade de Planos em desenvolvimento!')}>Planos</a></li>
              </ul>
            </div>
            <div>
              <h3 className="font-semibold mb-4">Contato</h3>
              <ul className="space-y-2 text-gray-400">
                <li><Mail className="inline-block w-4 h-4 mr-2" />mednexusoficial@gmail.com</li>
              </ul>
            </div>
          </div>
          <div className="border-t border-gray-800 mt-8 pt-8 text-center text-gray-400">
            <p>&copy; 2024 MedNexus. Todos os direitos reservados.</p>
          </div>
        </div>
      </footer>
    </div>
